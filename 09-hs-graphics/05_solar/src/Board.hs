...
boardFields = [ map fieldForChar row | row <- Config.boardData ]

fieldPicture Hall = Pictures.hall
fieldPicture Wall = Pictures.wall

rowPicture :: [Field] -> Picture
rowPicture fields = let combine = \ current previous -> pictures [ translate Config.blockSize 0 previous, current ]
